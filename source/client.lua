        ...
        if vehicleFueling then
            ...
            if usingCan then
                ...
                while vehicleFueling do
                    ...
                    if QBCore.Functions.GetPlayerData().money['cash'] < cost then
                        SendNUIMessage({
                            type = "warn"
                        })
                        vehicleFueling = false
        ...
        if pump then
            ...
            if not holdingNozzle and not nozzleInVehicle and not nozzleDropped then
                ...
                if IsControlJustPressed(0, 47) then
                    if QBCore.Functions.GetPlayerData().money['bank'] > price then
                        TriggerServerEvent("ND_Fuel:jerryCan", price)
                    else
                        QBCore.Functions.Notify("You don't have enough money in your bank", "error")
                    end
                end
...
--qbcore does not support jerrycan as weapon (yet)
--[[CreateThread(function()
    local wait = 500
    while true do
        Wait(wait)
...
RegisterNetEvent('ND_Fuel:useJerryCan', function() 
    if not IsPedInAnyVehicle(PlayerPedId(), false) then
        --[[XD.Functions.Progressbar("jerrycan_fill", "Refueling Vehicle", 9000, false, false, {
        ...
        CreateThread(function()
            ...
            while true do
                Wait(wait)
                if not holdingNozzle and not nozzleInVehicle then
                    wait = 0
                    local veh = vehicleInFront(ped, pedCoords)
                    if veh then
                        local vehClass = GetVehicleClass(veh)
                        ...
                        tankPosition = GetWorldPositionOfEntityBone(veh, tankBone)
                        if tankPosition and #(pedCoords - tankPosition) < 15 then
                            local fuel = GetFuel(veh)
                            DrawText3D(tankPosition.x, tankPosition.y, tankPosition.z + zPos, math.floor(fuel) .. "% Fuel")

                            if IsControlJustPressed(0, 51) then
                                if #(pedCoords - tankPosition) < 1.2 then
                                    QBCore.Functions.TriggerCallback('ND_Fuel:removeJerry', function(result)
                                        if result then
                                            refuelVehicle(veh)
                                        end
                                    end)
                                else
                                    QBCore.Functions.Notify("You need to get close to tank", "error")
                                end
                            end
                            if fuel >= 100 then
                                break
        ...
        QBCore.Functions.Notify("You need to be outside of vehicle to refuel it", "error")
    end 
end)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if not IsEntityPlayingAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 3) then
                LoadAnimDict("timetable@gardener@filling_can")
                TaskPlayAnim(ped, "timetable@gardener@filling_can", "gar_ig_5_filling_can", 2.0, 8.0, -1, 50, 0, 0, 0, 0)
            elseif DoesEntityExist(veh) then
                vehicleFueling = veh
                usingCan = true
            end
            if fuel >= 100 then
